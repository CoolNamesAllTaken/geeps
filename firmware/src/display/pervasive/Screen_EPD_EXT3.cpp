...
void Screen_EPD_EXT3_Fast::COG_MediumP_getDataOTP() {
    ...
    switch (u_codeDriver) {
        ...
        default:
            ...
            mySerial.println();
            mySerial.println(
                formatString("hV * OTP failed for screen %i-%cS-0%c", u_codeSize, u_codeFilm, u_codeDriver));
            while (0x01);
            break;
    }
    ...
    if (u_flagOTP == true) {
        ...
        mySerial.println();
        mySerial.println(
            formatString("hV * OTP check failed - First byte 0x%02x, expected 0x%04x", COG_data[0x00], _chipId));
        while (0x01);
    }
}
...
void Screen_EPD_EXT3_Fast::COG_SmallP_getDataOTP() {
    ...
    switch (u_eScreen_EPD) {
        ...
        default:
            ...
            mySerial.println(formatString("hV * OTP check failed - Screen %i-%cS-0%c not supported", u_codeSize,
                                          u_codeFilm, u_codeDriver));
            mySerial.flush();
            while (true);
            break;
    }
    ...
    if (offsetA5 > 0x0000) {
        ...
        if (ui8 != 0xa5) {
            mySerial.println();
            mySerial.println(
                formatString("hV * OTP check failed - Bank %i, first 0x%02x, expected 0x%02x", bank, ui8, 0xa5));
            mySerial.flush();
            while (0x01);
        }
    }
    ...
    switch (u_eScreen_EPD) {
        ...
        case eScreen_EPD_287_PS_09:
            ...
            mySerial.println(formatString("hV . OTP check passed - Bank %i, first 0x%02x %s", bank, ui8,
...
void Screen_EPD_EXT3_Fast::begin() {
    ...
    switch (u_codeSize) {
        ...
        default:
            ...
            mySerial.println();
            mySerial.println(
                formatString("hV * Screen %i-%cS-0%c is not supported", u_codeSize, u_codeFilm, u_codeDriver));
            while (0x01);
            break;
    }  // u_codeSize
    ...
    mySerial.println(formatString("hV = PDLS %s v%i.%i.%i", SCREEN_EPD_EXT3_VARIANT, SCREEN_EPD_EXT3_RELEASE / 100,
                                  (SCREEN_EPD_EXT3_RELEASE / 10) % 10, SCREEN_EPD_EXT3_RELEASE % 10));
    mySerial.println();
...
void Screen_EPD_EXT3_Fast::flush() { flushMode(UPDATE_FAST); }
...
void Screen_EPD_EXT3_Fast::s_setOrientation(uint8_t orientation) { v_orientation = orientation % 4; }
...
uint16_t Screen_EPD_EXT3_Fast::s_getPoint(uint16_t x1, uint16_t y1) { return 0x0000; }
//

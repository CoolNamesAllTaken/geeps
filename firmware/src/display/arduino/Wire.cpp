...
bool TwoWire::setSDA(pin_size_t pin) {
    ...
    //         __bitset({0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44}) /* I2C0 */,
    //         __bitset({2, 6, 10, 14, 18, 22, 26, 30, 34, 38, 42, 46}) /* I2C1 */
    //     };
    // #else
    //     constexpr uint64_t valid[2] = {
    //         __bitset({0, 4, 8, 12, 16, 20, 24, 28}) /* I2C0 */, __bitset({2, 6, 10, 14, 18, 22, 26}) /* I2C1 */
    //     };
    // #endif
...
bool TwoWire::setSCL(pin_size_t pin) {
    // #ifdef PICO_RP2350B
    //     constexpr uint64_t valid[2] = {
    //         __bitset({1, 5, 9, 13, 17, 21, 25, 29, 33, 37, 41, 45}) /* I2C0 */,
    //         __bitset({3, 7, 11, 15, 19, 23, 27, 31, 35, 39, 43, 47}) /* I2C1 */
    //     };
    // #else
    //     constexpr uint64_t valid[2] = {
    //         __bitset({1, 5, 9, 13, 17, 21, 25, 29}) /* I2C0 */, __bitset({3, 7, 11, 15, 19, 23, 27}) /* I2C1 */
    //     };
    // #endif
    //     if ((!_running) && ((1LL << pin) & valid[i2c_hw_index(_i2c)])) {
    //         _scl = pin;
...
size_t TwoWire::requestFrom(uint8_t address, size_t quantity) { return requestFrom(address, quantity, true); }
...
int TwoWire::available(void) { return _running ? _buffLen - _buffOff : 0; }
...
bool TwoWire::writeReadAsync(uint8_t address, const void *wbuffer, size_t wbytes, const void *rbuffer, size_t rbytes,
                             bool sendStop) {
    ...
    channel_config_set_irq_quiet(
        &c, false);  // Enable interrupt (can be disabled later with dma_channel_set_irq0_enabled() as needed)
...
bool TwoWire::finishedAsync() { return _dmaFinished; }
...
void TwoWire::onFinishedAsync(void (*function)(void)) { _dmaOnFinished = function; }
...
void TwoWire::onReceive(void (*function)(int)) { _onReceiveCallback = function; }

void TwoWire::onRequest(void (*function)(void)) { _onRequestCallback = function; }
...
bool TwoWire::getTimeoutFlag() { return _timeoutFlag; }

void TwoWire::clearTimeoutFlag() { _timeoutFlag = false; }

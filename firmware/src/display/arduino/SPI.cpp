...
SPIClassRP2040::SPIClassRP2040(spi_inst_t *spi, pin_size_t rx, pin_size_t cs, pin_size_t sck, pin_size_t tx) {
    ...
    _running = false;
    _initted = false;
    _spis = SPISettings(0, LSBFIRST, arduino::SPI_MODE0);  // Ensure spi_init called by setting current freq to 0
    _RX = rx;
    _TX = tx;
    _SCK = sck;
    _CS = cs;
...
uint16_t SPIClassRP2040::transfer16(uint16_t data) {
    ...
    hw_write_masked(&spi_get_hw(_spi)->cr0, (16 - 1) << SPI_SSPCR0_DSS_LSB,
...
void SPIClassRP2040::beginTransaction(SPISettings settings) {
    DEBUGSPI("SPI::beginTransaction(clk=%lu, bo=%s)\n", settings.getClockFreq(),
             (settings.getBitOrder() == MSBFIRST) ? "MSB" : "LSB");
...
bool SPIClassRP2040::transferAsync(const void *send, void *recv, size_t bytes) {
    ...
    dma_channel_configure(_channelSendDMA, &c, &spi_get_hw(_spi)->dr,
                          !send ? (uint8_t *)&_dummy : (_spis.getBitOrder() != MSBFIRST ? _dmaBuffer : txbuff), bytes,
                          false);
...
bool SPIClassRP2040::setRX(pin_size_t pin) {
    ...
    //         __bitset({0, 4, 16, 20, 32, 26}) /* SPI0 */, __bitset({8, 12, 24, 28, 40, 44}) /* SPI1 */
    //     };
    // #else
    //     constexpr uint64_t valid[2] = {
    //         __bitset({0, 4, 16, 20}) /* SPI0 */, __bitset({8, 12, 24, 28}) /* SPI1 */
    //     };
    // #endif
...
bool SPIClassRP2040::setCS(pin_size_t pin) {
    // #ifdef PICO_RP2350B
    //     constexpr uint64_t valid[2] = {
    //         __bitset({1, 5, 17, 21, 33, 37}) /* SPI0 */, __bitset({9, 13, 25, 29, 41, 45}) /* SPI1 */
    //     };
    // #else
    //     constexpr uint64_t valid[2] = {
    //         __bitset({1, 5, 17, 21}) /* SPI0 */, __bitset({9, 13, 25, 29}) /* SPI1 */
    //     };
    // #endif
    //     if ((!_running) && ((1LL << pin) & valid[spi_get_index(_spi)])) {
    //         _CS = pin;
...
bool SPIClassRP2040::setSCK(pin_size_t pin) {
    // #ifdef PICO_RP2350B
    //     constexpr uint64_t valid[2] = {
    //         __bitset({2, 6, 18, 22, 34, 38}) /* SPI0 */, __bitset({10, 14, 26, 30, 42, 46}) /* SPI1 */
    //     };
    // #else
    //     constexpr uint64_t valid[2] = {
    //         __bitset({2, 6, 18, 22}) /* SPI0 */, __bitset({10, 14, 26}) /* SPI1 */
    //     };
    // #endif
    //     if ((!_running) && ((1LL << pin) & valid[spi_get_index(_spi)])) {
    //         _SCK = pin;
...
bool SPIClassRP2040::setTX(pin_size_t pin) {
    // #ifdef PICO_RP2350B
    //     constexpr uint64_t valid[2] = {
    //         __bitset({3, 7, 19, 23, 35, 39}) /* SPI0 */, __bitset({11, 15, 27, 31, 43, 47}) /* SPI1 */
    //     };
    // #else
    //     constexpr uint64_t valid[2] = {
    //         __bitset({3, 7, 19, 23}) /* SPI0 */, __bitset({11, 15, 27}) /* SPI1 */
    //     };
    // #endif
    //     if ((!_running) && ((1LL << pin) & valid[spi_get_index(_spi)])) {
    //         _TX = pin;
...
void SPIClassRP2040::end() {
    ...
    _spis = SPISettings(0, LSBFIRST, arduino::SPI_MODE0);
}

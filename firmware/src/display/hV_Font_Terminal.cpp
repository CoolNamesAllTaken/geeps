...
uint16_t hV_Font_Terminal::_f_stringSizeX(char * text)
{
    return (uint16_t) strlen(text) * _f_font.maxWidth;
}

uint8_t hV_Font_Terminal::_f_stringLengthToFitX(char * text, uint16_t pixels)
{
    uint8_t index = 0;
